...
class NrsConnector @Inject()(appConfig: AppConfig, httpClient: HttpClient) {
  ...
  def submitNonRepudiation(
    ...
    ec: ExecutionContext): Future[NrsSubmissionResponse] =
    httpClient
      .POST[NRSSubmission, NrsSubmissionResponse](
        nrsSubmissionUrl,
        nrsSubmission,
        Seq[(String, String)](("Content-Type", "application/json"),
                              (xApiKeyHeader, appConfig.nrsApiKey))
      )
      .map { res =>
      ...
      .recoverWith {
        case httpError: HttpException =>
          logger.error(
            s"Call to nrs service failed url=$nrsSubmissionUrl, HttpException=$httpError")
          Future.failed(new RuntimeException(httpError))
        case e: Throwable =>
          logger.error(
            s"Call to nrs service failed url=$nrsSubmissionUrl, exception=$e")
          Future.failed(e)
      }
}
