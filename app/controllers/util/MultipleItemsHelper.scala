...
object MultipleItemsHelper {
  ...
  private def duplication[A](document: A, cachedData: Seq[A]): Seq[FormError] =
    if (cachedData.contains(document))
      Seq(FormError("", "supplementary.duplication"))
    else Seq.empty

  private def limitOfElems[A](limit: Int, cachedData: Seq[A]): Seq[FormError] =
    if (cachedData.length >= limit) Seq(FormError("", "supplementary.limit"))
    else Seq.empty
  ...
  /**
    ...
    */
  def remove[A](idOpt: Option[String], cachedData: Seq[A]): Seq[A] =
    idOpt match {
      case Some(id) if cachedData.length - 1 >= id.toInt =>
        removeItem(id, cachedData)
  ...
  def saveAndContinue[A](form: Form[A], cachedData: Seq[A], isMandatory: Boolean, limit: Int): Either[Form[A], Seq[A]] =
    if (!isFormEmpty(form)) add(form, cachedData, limit)
      ...
      if (mandatoryFieldsError.nonEmpty)
        Left(form.copy(errors = mandatoryFieldsError))
