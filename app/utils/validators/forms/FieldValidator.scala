...
object FieldValidator {
  ...
  val validDucr: String => Boolean = (input: String) =>
    input.matches("""^[0-9][A-Z][A-Z][0-9]{12}-[0-9A-Z\(\)\-/., \']{0,19}""")

  val validMucr: String => Boolean = (input: String) => input.matches("""^[A-Z][0-9A-Z\(\)\-/., \']{0,34}""")
