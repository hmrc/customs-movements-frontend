...
class DissociateUcrWithIleQueryDisabledISpec extends IntegrationSpec {
  ...
  "Dissociate UCR Page" when {
    "GET" should {
      "return 200" in {
        ...
        // When
        val response = get(DisassociateUcrController.displayPage)
    ...
    "POST" should {
      "continue" in {
        ...
        // When
        val response = post(DisassociateUcrController.submit, "kind" -> "mucr", "mucr" -> "GB/321-54321")

        // Then
        status(response) mustBe SEE_OTHER
        redirectLocation(response) mustBe Some(DisassociateUcrSummaryController.displayPage.url)
