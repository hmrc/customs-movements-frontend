...
class DisassociateUcrConfirmationControllerSpec extends ControllerLayerSpec with ScalaFutures {
  ...
  "Disassociate Ducr Confirmation Controller" should {
    ...
    "return 200 (OK)" in {
      ...
      verify(confirmationPage).apply(meq(JourneyType.DISSOCIATE_UCR), meq(None), meq(None))(any(), any())
    }
    ...
    "call FlashValuesExtractor" in {
      ...
      requestCaptor.getValue.flash.get(FlashKeys.JOURNEY_TYPE) mustBe Some(JourneyType.DISSOCIATE_UCR.toString)
    }
