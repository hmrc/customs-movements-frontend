...
trait MessagesStub {
  ...
  private class AllMessageKeysAreMandatoryMessages(msg: Messages) extends Messages {
    ...
    override def apply(key: String, args: Any*): String =
      ...
      else throw new AssertionError(s"Message Key is not configured for {$key} used in view")

    override def apply(keys: Seq[String], args: Any*): String =
      if (keys.exists(key => !msg.isDefinedAt(key)))
        msg.apply(keys, args)
      else throw new AssertionError(s"Message Key is not configured for {$keys} used in view")
